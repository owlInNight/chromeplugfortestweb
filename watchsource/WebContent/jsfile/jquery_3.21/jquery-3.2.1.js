 ...
 */
		...
		module.exports = global.document ?
			...
			function( w ) {
				if ( !w.document ) {
					/**
					 * jquery 需要一个带有 document 的window 类
					 * (requires 动词?)
					 */
...
/**
 * 这里是有一些问题的 传入 window  , 和 一个function 
 * 这里的this 指的是什么???
 * 什么情况下window 是空的 ??
 * 
 * 第二个参数  , 是一个 function 返回值是  jQuery;
 */
...
var Sizzle =
/*
 * ! Sizzle CSS Selector Engine v2.3.3 https://sizzlejs.com/
 * 
	...
	return function( elem ) {
		...
		if ( "form" in elem ) {
			...
			if ( elem.parentNode && elem.disabled === false ) {
				...
				// Support: IE 6 - 11
				// Use the isDisabled shortcut property to check for disabled
				// fieldset ancestors
...
jQuery.event = {
	...
	handlers: function( event, handlers ) {
		...
		if ( delegateCount &&
			...
			// Support: Firefox <=42
			// Suppress spec-violating clicks indicating a non-primary pointer
			// button (trac-3861)
